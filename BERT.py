...
def main():
    ...
    optim = NoamOpt(768, 0.1, len(tr_dl),
                    torch.optim.Adam(list(a.parameters()) + list(model.parameters()) + list(generator.parameters()),
    ...
    while epoch < args.max_epoch:
        ...
        for id_b, batch in enumerate(tr_dl):
            ...
            loss = F.pairwise_distance(pred[:, :].contiguous().view(-1, 2), torch.matmul(
                torch.cat((batch['src'][:, :, 2:4], batch['trg'][:, :, 2:4]), 1).contiguous().view(-1, 2).to(device),
        ...
        with torch.no_grad():
            ...
            for batch in val_dl:
                ...
                out = model(input_ids=net_input, position_ids=position, token_type_ids=token,
